...
class LecturerController extends Controller
{
    ...
    public function createSession(): void
    {
        ...
        $result = $this->sessionService->createSession($this->lecturerId, $payload);
        ...
        if (!$result['success']) {
            ...
            $this->audit(
                ...
                'sessions',
                "Failed session creation for course_id={$payload['course_id']} on {$payload['session_date']}"
            );
        ...
        $this->audit(
            'session.created', 
            'sessions',
            "Session ID {$result['session_id']} created for course_id={$payload['course_id']}"
        );
    ...
    public function activateSession(array $params): void
    {
        ...
        $this->audit('session.activated', 'sessions', "Session activated: session_id={$params['id']}");
    ...
    public function closeSession(array $params): void
    {
        ...
        $this->audit('session.closed', 'sessions', "Session closed: session_id={$params['id']}");
        $this->redirect('/lecturer/sessions');
    }
    ...
    public function manualAttendance(array $params): void
    {
        ...
        if (empty($regNo)) {
            ...
            $this->audit('attendance.manual_failed', 'attendance', "Empty reg number attempt on session_id=$sessionId");
        ...
        if (!$student) {
            $this->flash('error', "No student found with registration number: {$regNo}");
            $this->redirect('/lecturer/sessions/' . $sessionId . '/manual');
        }
        ...
        if ($exists) {
            $this->flash('error', "{$student['name']} has already been marked present.");
            $this->redirect('/lecturer/sessions/' . $sessionId . '/manual');
        }
        ...
        if ($classRep) {
            // Create pending manual request safely using parameter placeholders
            $this->db->insert(
                "INSERT INTO manual_attendance (session_id, student_id, reg_number, status, created_at)
                 VALUES (?, ?, ?, 'pending', NOW())",
                [$sessionId, $student['id'], $regNo]
            );
            $this->flash('success', "{$student['name']} — pending class rep confirmation.");
        } else {
            ...
            $this->flash('success', "Attendance recorded for {$student['name']}.");
        }

        $this->audit('attendance.manual', 'attendance', "Manual entry for student_id={$student['id']} on session_id=$sessionId");
        $this->redirect('/lecturer/sessions/' . $sessionId . '/manual');
    }

    private function audit(string $action, string $module, string $desc): void
    {
        require_once APP_PATH . '/services/AuditService.php';
        AuditService::record($action, $module, $desc);
    }
}